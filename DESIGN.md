...
```text
적용 가능한 DESIGN.md 계층을 임시 디자인 기준선으로 사용한다. 루트 DESIGN.md를 먼저 읽고, 대상 경로에 전용 DESIGN.md가 있으면 그 차이를 적용한다. YAML frontmatter의 토큰을 우선 적용하고, 본문 기준은 토큰을 어떻게 써야 하는지 설명하는 맥락으로 사용한다. 조용하고 작업 중심인 화면을 만들고, 중립적인 표면, 절제된 주요 액션, 명확한 위계, 안정적인 반응형 동작을 유지한다. 최종 브랜드 정체성을 임의로 만들지 않는다.
```
