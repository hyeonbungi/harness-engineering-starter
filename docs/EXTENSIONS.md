...
복사본을 두 벌 두면 drift가 생기므로 **정본은 `.codex/skills` 한 곳**에 두고, `.claude/skills`를 거기로 가는 **상대 symlink**(git에 mode 120000으로 커밋)로 만듭니다.
...
- 새 단일-파일 스킬: `.codex/skills/<name>`에만 추가합니다. directory symlink 덕분에 Claude와 Codex에 동시에 반영되고, 중복·drift가 생기지 않습니다.
...
- `init.sh`는 `.codex/skills/*/SKILL.md` frontmatter를 검증하고, `.claude/skills` symlink는 비치명적 warning으로만 확인합니다.
...
|---|---|---|---|---|
| `design-md` | `DESIGN.md` 작성, 감사, migration, 선택 검증 | `.claude/skills/design-md/SKILL.md` via symlink | `.codex/skills/design-md/SKILL.md` | Local policy using `google-labs-code/design.md` (Apache-2.0) and `VoltAgent/awesome-design-md` (MIT) references |
